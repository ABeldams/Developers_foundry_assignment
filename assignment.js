...
const newUserTotal = users.map(user => ({
  ...user,
  Total:sum(user.productsInCart)
...
const newUserTotal3500 = newUserTotal.map(user => ({
  ...user,
	Total:sum(user.productsInCart),
  
})).filter(user => user.Total > 3500);
...
const productQuantityOf2=users.map(user=>({
   ...user,
   
   productsInCart: user.productsInCart.filter(
...
console.log(newUserTotal3500);
console.log(fullNameArray);
console.log(productQuantityOf2);
